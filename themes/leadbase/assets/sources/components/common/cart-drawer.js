...
(function () {
  ...
  async function render() {
      ...
      itemsContainer.innerHTML = `
        <div class="text-center py-12 flex flex-col items-center justify-center h-full">
          <div class="w-24 h-24 bg-blue-50 rounded-full flex items-center justify-center mb-6 shadow-inner border border-blue-100">
            <svg class="mx-auto h-10 w-10 text-blue-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
            </svg>
          </div>
          <p class="text-slate-900/80 font-medium text-lg">Giỏ hàng đang trống</p>
          <p class="text-slate-900/50 mt-2 text-sm">Hãy khám phá thêm sản phẩm nhé!</p>
        </div>
      `;
    ...
    const rows = cart.map((item, index) => {
      ...
      const imageUrl = (p.imageUrls && p.imageUrls[0]) || 'https://placehold.co/100x100/E8ECF0/64748B?text=No+Image';

      return `
        <li class="flex py-6 relative z-10 group/item">
          <div class="h-24 w-24 flex-shrink-0 overflow-hidden rounded-[20px] border border-slate-900/5 bg-white shadow-sm relative group-hover/item:shadow-md transition-shadow">
          ...
          <div class="ml-4 flex flex-1 flex-col">
            <div>
              <div class="flex justify-between text-base font-medium text-slate-950">
                <h3 class="font-sans line-clamp-2 pr-4"><a href="/product/${p.slug || p.id}" class="hover:text-blue-600 transition-colors">${label}</a></h3>
                <p class="ml-4 whitespace-nowrap text-blue-700 font-medium">${formatPrice(unitPrice)}</p>
              </div>
              ${variantLabel ? `<p class="mt-1 text-sm text-slate-900/60">${variantLabel}</p>` : ''}
            </div>
            <div class="flex flex-1 items-end justify-between text-sm mt-3">
              <div class="flex items-center border border-slate-900/10 rounded-full bg-white shadow-sm overflow-hidden h-8">
                <button type="button" class="cart-qty-btn px-3 h-full text-blue-700 hover:bg-blue-50 transition-colors flex items-center justify-center" data-index="${index}" data-action="minus">
                  <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4"></path></svg>
                </button>
                <span class="px-2 min-w-[2rem] text-center text-slate-950 font-medium">${item.quantity}</span>
                <button type="button" class="cart-qty-btn px-3 h-full text-blue-700 hover:bg-blue-50 transition-colors flex items-center justify-center" data-index="${index}" data-action="plus">
                  <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
                </button>
              </div>

              <div class="flex">
                <button type="button" data-index="${index}" class="cart-remove p-2 -mr-2 text-slate-900/40 hover:text-red-500 hover:bg-red-50 rounded-full transition-all" title="Xoá">
    ...
    itemsContainer.innerHTML = `<ul role="list" class="-my-6 divide-y divide-slate-900/5">${rows.join("")}</ul>`;
