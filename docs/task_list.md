...
- [x] Retry khi gọi lỗi — `services/orderRetry.ts`, `node-cron` mỗi 5 phút quét `CartOrder.status='failed'` retry lại. KHÔNG có cột đếm số lần retry trong schema (tránh migration) — dùng tuổi đơn (`createdAt` < 24h) làm giới hạn thay vì đếm lượt. **Bug phát hiện qua test thật trên VPS**: cron cần biết domain của chính instance để gửi lại, lấy từ `SiteConfig.singleton` — nhưng row đó trước đây chỉ được tạo lười ở `/admin/settings/general` (lần đầu ai đó vào Settings), nên đơn `failed` trên 1 instance chưa từng có ai vào Settings sẽ bị cron bỏ qua VĨNH VIỄN (`resolveDomain()` trả `null`). Sửa bằng `services/siteConfig.ts` (`getOrCreateSiteConfig`) dùng chung ở cả `settings.ts` lẫn `cart.ts` — singleton giờ được tạo ngay từ đơn hàng ĐẦU TIÊN, không phụ thuộc admin đã ghé Settings hay chưa.
...
## Phase 5.5 — Sản phẩm biến thể (variant), vd màu/size

Nguyên tắc: **additive**, sản phẩm không bật `has_variants` đi nguyên xi qua mọi code path cũ (>95% sản phẩm hiện có, rủi ro 0). Kế hoạch đầy đủ + rà soát 3 endpoint tạo đơn của LeadBase: `C:\Users\hieud\.claude\plans\playful-juggling-crab.md`.

- [x] **[lead-base]** Migration `products.has_variants` (default false) + bảng `product_variants` (`product_id`, `sku`, `attributes` JSON, `price`, `sale_price`, `stock`, `status`) + `order_items.product_variant_id` (nullable FK, `product_id` đã nullable sẵn nên an toàn). Model `ProductVariant`, quan hệ `Product::variants()`, `OrderItem::variant()`.
- [x] **[lead-base]** `ProductVariant::booted()` — mỗi lần lưu/xoá variant: (1) tự tính lại `Product.price` = giá thấp nhất, `Product.stock` = tổng tồn các variant (`saveQuietly()` tránh vòng lặp sync) — để `OrderController` (tạo đơn tay CRM) và `OrderIntakeController` (API marketing match theo tên) vẫn đọc `Product.price/stock` như cũ mà KHÔNG vỡ (giá `0`/`null`); (2) gọi `ProductSyncService::syncToAllWebsites($product, 'update')` — dùng lại đúng method có sẵn, không tạo method riêng.
- [x] **[lead-base]** UI quản trị: checkbox "Sản phẩm có biến thể" + `ProductVariantsEditor.tsx` (bảng thêm/sửa/xoá variant, lưu ngay qua axios — tách khỏi form Inertia chính) trong `Products/Edit.tsx`. Route CRUD riêng `ProductVariantController.php` (`GET/POST /products/{product}/variants`, `PUT/DELETE .../{variant}`).
- [x] **[lead-base]** `SyncProductToWebsite.php` — payload kèm `variants[]` (`leadbaseVariantId`, `sku`, `attributes`, `price`, `salePrice`, `stock`, `status`) khi `product.has_variants`, KHÔNG thay thế price/stock top-level (tương thích ngược).
- [x] site-engine: `ProductVariantCache` (Prisma) — con của `ProductCache` (`hasVariants Boolean`), `leadbaseVariantId @unique`. `productsSync.ts` nhận `variants[]` optional ở cả `create`/`update`, upsert theo `leadbaseVariantId` (idempotent, giống trick chống trùng của product).
- [x] site-engine: `/admin/products/:id` hiện bảng variant read-only khi `hasVariants` (khác trang thường — không có form sửa giá/tồn, vì đây là dữ liệu LeadBase sở hữu).
- [x] site-engine: `themes/default/product-detail.liquid` — bộ chọn variant (dropdown theo từng thuộc tính, build từ `attributes` các variant qua JS phía client, dữ liệu nhúng an toàn qua `<script type="application/json">` có escape `</script>`), đổi giá/tồn/khoá nút mua theo lựa chọn. `cart.liquid`/`cart.ts` — cart item thêm `variantId` optional, xoá theo INDEX thay vì `productId` (2 dòng có thể cùng sản phẩm khác variant). Checkout resolve giá THẬT từ `ProductVariantCache`, tên hiển thị ghép thuộc tính (vd "Áo thun - Màu: Đỏ, Size: M").
- [x] **[lead-base]** `SiteEngineOrderController` nhận `items.*.leadbaseVariantId` optional, set `OrderItem.product_variant_id` (cast lại `int`, giá trị nhận vào là string), `product_id` LUÔN là sản phẩm cha (để chỗ cũ query theo `product_id` không vỡ).
- [x] **[lead-base]** 3 việc từng ghi "ngoài phạm vi" ở trên — làm nốt theo yêu cầu sau đó: `OrderController` (tạo đơn tay CRM, `Orders/Create.tsx`/`Edit.tsx`) — bấm sản phẩm có variant mở bảng con chọn variant (khoá nút nếu hết hàng), `ProductController::search()` trả kèm `variants[]`; `OrderIntakeController` (API marketing match theo tên tự do) — thử khớp thuộc tính (màu/size) trong phần chữ còn lại sau khi khớp tên sản phẩm, không chắc thì giữ giá sản phẩm gốc thay vì đoán bừa; Chatbot AI (`BuildsChatbotContext::productContext()`) — thêm field `bien_the` gọn (`id(Mau:Do,Size:M)=giá{Hết hàng}`) vào ngữ cảnh, prompt bắt buộc AI hỏi khách chọn variant trước khi điền `variant_id`, không tự đoán. Cả 3 verify qua tinker/reflection trước khi deploy. **Vẫn chưa làm**: `ProductSetPicker.tsx` (marketing) chưa hiện variant.
...
- [x] **[lead-base]** "Xoá Website" — đã làm từ Phase 1 (`WebsiteProvisionService::remove()` gọi `site-engine-provision-app.sh remove` + `site-engine-provision-domain.sh remove`: `systemctl disable --now`, gỡ Nginx vhost, `dropdb`, `rm -rf` thư mục app), `WebsiteController::destroy()` gọi `remove()` rồi mới xoá row. `Websites.tsx` có `window.confirm` cảnh báo rõ "app + database sẽ bị xoá vĩnh viễn". Mục này bị bỏ sót chưa đánh dấu `[x]` dù đã xong — không phải việc còn thiếu.
