...
- [ ] **Ngoài phạm vi đợt này (đã ghi rõ trong kế hoạch, không âm thầm bỏ qua)**: chatbot AI product context, `OrderController` (tạo đơn tay CRM), `OrderIntakeController` (API marketing) — cả 3 chưa có UI/logic CHỌN variant cụ thể, `OrderItem.product_variant_id` luôn `null` với đơn tạo từ 2 chỗ sau; `ProductSetPicker.tsx` (marketing) chưa hiện variant.
