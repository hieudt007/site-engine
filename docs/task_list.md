...
- [x] UI soạn bài: list + editor (`routes/admin/postsUi.ts` + `views/admin/{posts-list,post-edit}.liquid`) — server-render HTML, gọi JSON API ở `/admin/api/posts*` bằng `fetch()` client-side. Editor là `<textarea>` thường (rich text lib vẫn TBD, chưa chọn) — lưu HTML/markdown thô, khớp docblock `Post.body` ("đã sanitize" — sanitize thật sự CHƯA làm, ghi nhận nợ kỹ thuật).
