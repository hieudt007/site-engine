...
- [x] **[lead-base]** `WebsiteProvisionService.php` (mirror `LandingDomainProvisionService`): bung `site-engine.zip` vào `/var/www/site-engine/{domain}`, `npm ci --omit=dev`, `createdb`, sinh `SITE_ENGINE_SECRET` ngẫu nhiên lưu `Website.secret`, ghi `.env` (qua file tạm, tránh lộ secret trong `ps aux`), `prisma migrate deploy`, `systemctl enable --now`, Nginx vhost — code viết gộp luôn cả 2 bước systemd + nginx (Phase 2) trong 1 lần thay vì tách riêng như dự tính ban đầu.
...
- [x] `systemd/site-engine-instance@.service` (`tech_doc.md` §5) — `EnvironmentFile=/var/www/site-engine/%i/.env` (đảo từ `/etc/` sang trong thư mục app sau khi gặp lỗi thật "Read-only file system" trên 1 VPS test — `/etc` bị mount read-only).
...
- [x] Đổi định danh chính từ `websiteId` (số) sang **`domain`** cho thư mục app/systemd instance — mirror đúng quy ước Landing Page (`/var/www/{domain}`), theo yêu cầu người dùng khi test thật; giữ namespace `/var/www/site-engine/{domain}` (không đặt phẳng `/var/www/{domain}`) để không đụng độ nếu 1 domain trùng giữa Landing Page và Website. DB name = domain viết lại bằng gạch dưới (`site_engine_blog_leadbase_vn`), `id` chỉ còn dùng nội bộ để cấp port.
